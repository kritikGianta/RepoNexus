...
import { FileText, GitBranch, Loader2, Download, Copy, Check } from "lucide-react"
...
export default function ReleaseNotesPage() {
  ...
  const cacheKey = activeRepo ? `releasenotes-${activeRepo.id}` : ''
  const cached = history.get(cacheKey)

  const generateMutation = useMutation({
    mutationFn: (repoId: number) =>
      api.post<{ markdown_content: string }>(`/helpers/repos/${repoId}/release-notes`),
    onSuccess: (data) => {
    ...
    <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-surface-500 mt-1">
            Convert your messy commit history into a clean, professional changelog.
          </p>
        </div>
      ...
      {!content && !generateMutation.isPending && (
        <Tilt tiltMaxAngleX={5} tiltMaxAngleY={5} scale={1.02} transitionSpeed={2000} glareEnable={true} glareMaxOpacity={0.05} glareColor="#0ea5e9" glarePosition="all" className="perspective-1000">
          <div className="card text-center py-16 preserve-3d">
            ...
            <h2 className="text-lg font-bold text-surface-900 mb-2">Draft Release Notes</h2>
            <p className="text-surface-500 text-sm max-w-md mx-auto mb-6">
              We'll analyze the last 30 days of commits in <span className="font-semibold text-surface-700">{activeRepo?.full_name || 'your repository'}</span> and generate a categorized markdown changelog.
            </p>
      ...
      {generateMutation.isPending && (
        <div className="card text-center py-16">
          ...
          <h2 className="text-lg font-bold text-surface-900 mb-1">Analyzing commits...</h2>
          <p className="text-surface-500 text-sm">Translating developer jargon into professional release notes.</p>
        </div>
      )}
      ...
      {content && (
        <div className="card">
          <div className="flex items-center justify-between mb-6 border-b border-surface-100 pb-4">
            <h2 className="font-bold text-surface-900">Release Notes</h2>
            <div className="flex gap-2">
              <button onClick={handleCopy} className="btn-secondary text-xs py-1.5 px-3">
                {copied ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                {copied ? 'Copied' : 'Copy'}
              </button>
              <button onClick={() => exportMarkdown(content, `${activeRepo?.full_name.split('/')[1] || 'repo'}-CHANGELOG.md`)} className="btn-secondary text-xs py-1.5 px-3">
      ...
      {generateMutation.isError && (
        <div className="card border-red-200 bg-red-50">
          <p className="text-sm text-red-600">{generateMutation.error instanceof Error ? generateMutation.error.message : 'Failed to generate release notes.'}</p>
