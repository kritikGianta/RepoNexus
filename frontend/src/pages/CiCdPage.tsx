...
import { Settings, GitBranch, Loader2, Download, Copy, Check } from "lucide-react"
...
export default function CiCdPage() {
  ...
  const cacheKey = activeRepo ? `cicd-${activeRepo.id}` : ''
  const cached = history.get(cacheKey)

  const generateMutation = useMutation({
    mutationFn: (repoId: number) =>
      api.post<{ markdown_content: string }>(`/helpers/repos/${repoId}/cicd-gen`),
    onSuccess: (data) => {
    ...
    <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-surface-500 mt-1">
            Generate a secure, production-ready GitHub Actions workflow for your repository.
          </p>
        </div>
      ...
      {!content && !generateMutation.isPending && (
        <Tilt tiltMaxAngleX={5} tiltMaxAngleY={5} scale={1.02} transitionSpeed={2000} glareEnable={true} glareMaxOpacity={0.05} glareColor="#0ea5e9" glarePosition="all" className="perspective-1000">
          <div className="card text-center py-16 preserve-3d">
            <div className="w-16 h-16 rounded-2xl bg-sky-50 flex items-center justify-center mx-auto mb-5 animate-float-icon">
              <Settings className="w-8 h-8 text-sky-500" />
            </div>
            <h2 className="text-lg font-bold text-surface-900 mb-2">Generate CI/CD Pipeline</h2>
            <p className="text-surface-500 text-sm max-w-md mx-auto mb-6">
              We'll analyze the tech stack in <span className="font-semibold text-surface-700">{activeRepo?.full_name || 'your repository'}</span> and build a complete workflow with tests and linting.
            </p>
            <button onClick={handleGenerate} disabled={!activeRepo} className="btn-primary">
              <Settings className="w-4 h-4" /> Generate Pipeline
            </button>
          </div>
        </Tilt>
      )}
      ...
      {generateMutation.isPending && (
        <div className="card text-center py-16">
          ...
          <h2 className="text-lg font-bold text-surface-900 mb-1">Building workflow...</h2>
          <p className="text-surface-500 text-sm">Analyzing dependencies and structuring GitHub Actions.</p>
        </div>
      )}
      ...
      {content && (
        <div className="card">
          <div className="flex items-center justify-between mb-6 border-b border-surface-100 pb-4">
            <h2 className="font-bold text-surface-900">Generated Pipeline</h2>
            <div className="flex gap-2">
              <button onClick={handleCopy} className="btn-secondary text-xs py-1.5 px-3">
                {copied ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                {copied ? 'Copied' : 'Copy'}
              </button>
              <button onClick={() => exportMarkdown(content, `${activeRepo?.full_name.split('/')[1] || 'repo'}-CI-CD.md`)} className="btn-secondary text-xs py-1.5 px-3">
      ...
      {generateMutation.isError && (
        <div className="card border-red-200 bg-red-50">
          <p className="text-sm text-red-600">{generateMutation.error instanceof Error ? generateMutation.error.message : 'Failed to generate pipeline.'}</p>
