...
import { FileCode, GitBranch, Loader2, Download, Copy, Check } from "lucide-react"
...
export default function ReadmePage() {
  ...
  const cacheKey = activeRepo ? `readme-${activeRepo.id}` : ''
  const cached = history.get(cacheKey)

  const generateMutation = useMutation({
    mutationFn: (repoId: number) =>
      api.post<{ markdown_content: string }>(`/helpers/repos/${repoId}/readme-gen`),
    onSuccess: (data) => {
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="border-b border-surface-200/60 pb-6">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-surface-900 flex items-center gap-2.5">
          <FileCode className="w-7 h-7 text-brand-500" />
          README Generator
        </h1>
        <p className="text-sm text-surface-500 mt-2">
          Auto-generate comprehensive and professional README documentation from your codebase.
        </p>
      </div>
      ...
      {!content && !generateMutation.isPending && (
        <Tilt tiltMaxAngleX={5} tiltMaxAngleY={5} scale={1.02} transitionSpeed={2000} glareEnable={true} glareMaxOpacity={0.05} glareColor="#6366f1" glarePosition="all" className="perspective-1000">
          <div className="card text-center py-16 preserve-3d">
            <div className="w-16 h-16 rounded-2xl bg-brand-50 flex items-center justify-center mx-auto mb-5 animate-float-icon">
              <FileCode className="w-8 h-8 text-brand-500" />
            </div>
            <h2 className="text-lg font-bold text-surface-900 mb-2">Generate a README</h2>
            <p className="text-surface-500 text-sm max-w-md mx-auto mb-6">
              We'll analyze the file structure and config files of <span className="font-semibold text-surface-700">{activeRepo?.full_name || 'your repository'}</span> to create a publication-quality README.
            </p>
            <button onClick={handleGenerate} disabled={!activeRepo} className="btn-primary">
              <FileCode className="w-4 h-4" /> Generate README
            </button>
          </div>
        </Tilt>
      )}
      ...
      {generateMutation.isPending && (
        <div className="card text-center py-16">
          ...
          <h2 className="text-lg font-bold text-surface-900 mb-1">Generating README...</h2>
          <p className="text-surface-500 text-sm">Analyzing repo structure and writing documentation.</p>
        </div>
      )}
      ...
      {content && (
        <div className="card">
          <div className="flex items-center justify-between mb-6 border-b border-surface-100 pb-4">
            <h2 className="font-bold text-surface-900">Generated README.md</h2>
            <div className="flex gap-2">
              <button onClick={handleCopy} className="btn-secondary text-xs py-1.5 px-3">
                {copied ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                {copied ? 'Copied' : 'Copy'}
              </button>
              <button onClick={() => exportMarkdown(content, `${activeRepo?.full_name.split('/')[1] || 'repo'}-README.md`)} className="btn-secondary text-xs py-1.5 px-3">
      ...
      {generateMutation.isError && (
        <div className="card border-red-200 bg-red-50">
          <p className="text-sm text-red-600">{generateMutation.error instanceof Error ? generateMutation.error.message : 'Failed to generate README.'}</p>
